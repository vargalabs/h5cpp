...
TEST_CASE("H5Tmeta enable_or and enable_and participate correctly") {
    CHECK((std::is_same_v<h5::meta::enable_or<std::true_type>, int>));
    CHECK((std::is_same_v<h5::meta::enable_or<std::false_type, std::true_type>, int>));
    CHECK((std::is_same_v<h5::meta::enable_and<std::true_type>, int>));
    CHECK((std::is_same_v<h5::meta::enable_and<std::true_type, std::true_type>, int>));

    CHECK_FALSE((h5::compat::is_detected_v<enable_or_op_t, std::false_type>));
    CHECK_FALSE((h5::compat::is_detected_v<enable_and_op_t, std::true_type, std::false_type>));
}
...
TEST_CASE("H5Tmeta size counts scalar elements in mixed nested containers") {
    // Known limitation:
    // size(...) currently reports only outer container extent for nested std::array.    
