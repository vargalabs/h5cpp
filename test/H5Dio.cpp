...
TEST_CASE_TEMPLATE_DEFINE("[#114] h5 round-trip", T, io_round_trip) {
    ...
    if constexpr (rep == sr_t::scalar) {
        WARN("not yet implemented: scalar round-trip for " << type_name);
    } else if constexpr (rep == sr_t::c_array) {
        WARN("not yet implemented: c_array round-trip for " << type_name);
    } else if constexpr (rep == sr_t::linear_value_dataset) {
        WARN("not yet implemented: linear_value_dataset round-trip for " << type_name);
    } else if constexpr (rep == sr_t::ragged_vlen_dataset) {
        WARN("not yet implemented: ragged_vlen_dataset round-trip for " << type_name);
    } else if constexpr (rep == sr_t::fixed_inner_extent_dataset) {
        WARN("not yet implemented: fixed_inner_extent_dataset round-trip for " << type_name);
    } else if constexpr (rep == sr_t::key_value_dataset) {
        WARN("not yet implemented: key_value_dataset round-trip for " << type_name);
    } else if constexpr (rep == sr_t::vlen_text_dataset) {
        WARN("not yet implemented: vlen_text_dataset round-trip for " << type_name);
    }
}
